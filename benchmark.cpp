    ...
    for (const auto &kernel : kernels)
    {
        for (int downsample = 1; downsample <= MAX_DOWNSAMPLE; ++downsample)
        {
            ...
            for (int iteration = 0; iteration < ITERATIONS; ++iteration)
            {
                ...
                start = std::chrono::high_resolution_clock::now();
                GpuOperations::erodeImage(GpuOperations::Base, downsampledImage, kernelResized);
                end = std::chrono::high_resolution_clock::now();
                durationCudaBase.push_back(std::chrono::duration_cast<std::chrono::microseconds>(end - start).count());

                start = std::chrono::high_resolution_clock::now();
                GpuOperations::erodeImage(GpuOperations::Halo, downsampledImage, kernelResized);
                end = std::chrono::high_resolution_clock::now();
                durationCudaHalo.push_back(std::chrono::duration_cast<std::chrono::microseconds>(end - start).count());

                start = std::chrono::high_resolution_clock::now();
                GpuOperations::erodeImage(GpuOperations::Extern, downsampledImage, kernelResized);
                end = std::chrono::high_resolution_clock::now();
            ...
            std::cout << "Writing results to: " << result_folder << std::endl;

            writeResults(result_folder / "seq_results.txt", durationSeq);
            writeResults(result_folder / "parallel_results.txt", durationParallel);
            writeResults(result_folder / "cuda_base_results.txt", durationCudaBase);
            writeResults(result_folder / "cuda_halo_results.txt", durationCudaHalo);
            writeResults(result_folder / "cuda_extern_results.txt", durationCudaExtern);
